...
from db_inventory.models.base import PublicIDModel, PublicIDRegistry
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for model in apps.get_models():
            ...
            # Only process PublicIDModel subclasses
            ...
            qs = model.objects.exclude(public_id__isnull=True).values_list(
                "public_id", flat=True
            )

            buffer = []

            for public_id in qs.iterator(chunk_size=batch_size):
    ...
    def _flush(self, public_ids, model_label, dry_run):
        existing_ids = set(
            PublicIDRegistry.objects.filter(public_id__in=public_ids)
        ...
        with transaction.atomic():
            PublicIDRegistry.objects.bulk_create(
                to_create,
                ignore_conflicts=True,  # safety for concurrent runs
            )
